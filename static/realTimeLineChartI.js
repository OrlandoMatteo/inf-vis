//CURRENT
function realTimeLineChartI() {
  var margin = { top: 20, right: 20, bottom: 20, left: 40 }, width = 600, height = 400, duration = 500, color = d3.schemeCategory20b;
  function chart(selection) {
    // Based on https://bl.ocks.org/mbostock/3884955
    selection.each(function (data) {
      data = ["I"].map(function (c) {
        return {
          label: c,
      ...
      var t = d3.transition().duration(duration).ease(d3.easeLinear), x = d3.scaleTime().rangeRound([0, width - margin.left - margin.right]), y = d3.scaleLinear().rangeRound([height - margin.top - margin.bottom, 0]), z = d3.scaleOrdinal(color);
